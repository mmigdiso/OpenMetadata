...
public class WorkflowUtil {
  ...
  public static boolean setGlossaryTermStatusApproved(GlossaryTerm original, String updatedBy) {
    try {
      ...
      JsonPatch patch =
          JsonUtils.getJsonPatch(JsonUtils.pojoToJson(updated), JsonUtils.pojoToJson(original));
      ...
      LOG.info(
          "Auto-approved glossary term '{}' by reviewer '{}'",
          original.getFullyQualifiedName(),
          updatedBy);
      return true;
    } catch (Exception e) {
      LOG.error(
          "Failed to auto-approve glossary term {}: {}, Falling back to workflow",
          original.getId(),
          e.getMessage(),
          e);
      return false;
  ...
  public static boolean shouldTriggerGlossaryTermWorkflow(UUID entityId, String updatedBy) {
    ...
    boolean isReviewer =
        term.getReviewers() != null
            && term.getReviewers().stream()
                .anyMatch(ref -> ref.getName() != null && ref.getName().equals(updatedBy));
    if (isReviewer) {
